...
def setup_parser(subparser):
    ...
    # Arguments for spack-sdploy extension
    create_parser.add_argument(
        '--prefix', help="")
    create_parser.add_argument(
        '-s', '--stack', help="")
...
def mirror_create(args):
    ...
    # Instantiate ModulesYaml class. This will also create
    # the `mirrors` dictionary contaning the full paths to
    # the mirrors defined in commons.yaml.
    mirrors = MirrorsYaml(config)

    args.directory = mirrors.mirrors['paths']['local']
    # List of all specs (possibily read from the environment)'
    mirror_specs = _determine_specs_to_mirror(args)

    # Mirror object
    # .push_url = location to store the tarballs
    mirror = spack.mirror.Mirror(
        args.directory or spack.config.get('config:source_cache'))

    # directory is a string that resolves to the location where the
    # mirror will be stored: file:///work/stack/spack-mirror.
    # `args.directory = /work/stack/spack-mirror`
    directory = url_util.format(mirror.push_url)

    # Boolean: does directory already exists ?
    existed = web_util.url_exists(directory)

    # Actually do the work to create the mirror.
    # present, mirrored and error are all lists.
