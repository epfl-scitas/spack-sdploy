...
if [[ $option = "help" ]]; then
    echo "setupenv       - export environment variables and exit"
    echo "concretize     - run till the concretization step (inclusive)"
    return
fi

# CONFIGURATION
# USE staxk/intel directory
export STACK=intel
...
elapsed=$(( $end - $start ))
echo "> step 1 took $elapsed seconds"
...
elapsed=$(( $end - $start ))
echo "> step 2 took $elapsed seconds"
...
elapsed=$(( $end - $start ))
echo "> step 3 took $elapsed seconds"
...
elapsed=$(( $end - $start ))
echo "> step 4 took $elapsed seconds"
...
elapsed=$(( $end - $start ))
echo "> step 5 took $elapsed seconds"
...
elapsed=$(( $end - $start ))
echo "> step 6 took $elapsed seconds"
if [[ $option = "compilers" ]]; then
    echo "program stopped at install compilers step"
    return
fi
...
elapsed=$(( $end - $start ))
echo "> step 7 took $elapsed seconds"
...
echo
. ${_PREFIX}/activate_packages.sh 2>&1 | tee ${LOGS}/10_activate_packages.${execution_timestamp}.log
